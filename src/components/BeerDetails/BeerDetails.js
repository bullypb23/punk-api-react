...
import { 
  Wrapper, Paragraph, Container, ImageContainer, Image, DetailsContainer, Heading, ListItem, Span, SmallParagraph 
} from './BeerDetails.styles';
import * as actions from '../../store/actions';
import noImage from '../../assets/no-image.png';

const BeerDetails = ({ match, handleFetchBeerDetails, beer }) => {
  useEffect(() => {
  ...
  if (Object.keys(beer).length !== 0) {
    ...
    item = (
      <Container>
        <Paragraph>There is no beer selected!</Paragraph>
      </Container>
    )
  }
